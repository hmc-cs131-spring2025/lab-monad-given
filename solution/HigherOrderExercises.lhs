...
Hello! This file is "Literate Haskell". In Literate Haskell, most of the
information in the file is text. Every now and then, some Haskell code will
appear. When it does, it appears after the > character, like so:

> identity :: a -> a
> identity x = x

You can load a literate Haskell file into ghci, just like any other Haskell
file. Try loading this file now. (It's okay, we'll wait.)
...
EXERCISE: Using the function-composition operator (.) and pointfree style, along
          with functions we haveve defined in this file, fill in the definition 
...
We could define f in pointfree style like so:
...
SOLUTION: There are many expressions that will do the work we want. Here is one:

              map (fmap (show . double)) [Just 1, Just 2, Just 3]
